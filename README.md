...
| **6** | **Analyst-Facing Dashboard** | [`static/index.html`](file:///c:/Users/Dell/Desktop/Al-Powered%20Behavioral%20Anomaly%20Detection/static/index.html), [`static/app.js`](file:///c:/Users/Dell/Desktop/Al-Powered%20Behavioral%20Anomaly%20Detection/static/app.js), [`src/dashboard/dashboard_service.py`](file:///c:/Users/Dell/Desktop/Al-Powered%20Behavioral%20Anomaly%20Detection/src/dashboard/dashboard_service.py) | ✅ Verified (Glassmorphism Web UI, risk score 0-100, live WebSocket) |
...
- **Graph Node Feature Matrix Constructor**: Converts unstructured entity interaction logs into normalized feature matrices $X \in \mathbb{R}^{N \times F}$ and topological edge adjacency tensors $E \in \mathbb{R}^{2 \times M}$.
...
- **Encoder**: 2-layer Bidirectional LSTM projecting input sequence $X_{1:T}$ into latent vector $z \in \mathbb{R}^d$.
- **Decoder**: Unrolls latent vector $z$ back to reconstruct sequence $\hat{X}_{1:T}$.
- **Anomaly Score Formulation**: Reconstructive Mean Squared Error (MSE) loss per sequence sample:
  $$\text{Loss}_{\text{reconstruction}} = \frac{1}{T} \sum_{t=1}^T \| X_t - \hat{X}_t \|_2^2$$
...
- **Tree Stability Selection**: Random Forest ensemble with bootstrap resamples ($N=50$) measuring feature selection frequencies.
...
- **Principal Component Analysis (PCA)**: Linear orthogonal projection preserving maximum variance with explained variance ratio analysis.
...
### 7. Time Series Splitting & Cross-Validation
- **AdvancedTimeSeriesSplit**: Rolling window, expanding window, and blocked temporal splitting preventing lookahead leakage.
- **Stratified K-Fold**: 5-Fold cross-validation preserving exact class distributions across folds.

### 8. Overfitting & Underfitting Controls
- **EarlyStopping**: Monitors validation loss plateaus with patience thresholds and automatic best-checkpoint restoration.
- **ModelMonitor**: Loss trajectory diagnostic tool detecting capacity underfitting or generalization overfitting.
- **RegularizationManager**: Automated Grid and Random searches for optimal Ridge/Lasso alphas and ElasticNet L1 ratios.
